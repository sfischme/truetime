function [exectime, data] = blueCtrlcode(seg, data)

global xPos yPos xVel yVel xPos_middle yPos_middle oldBallZone tellArrive noticeBall
...
switch seg,
 ...
 case 1,
  ...
  if ~isempty(msg)
    ...
    elseif(msg.xRef==3000)
      ...
      data.xref = msg.xRef;
      data.yref = msg.yRef;
      %sprintf('Mote:%d, x: %d, y: %d',data.nodeID,data.xref,data.yref)
 ...
 case 2, 
  ...
  % Check sensor
  [obst, nodePos, ballSeen, ballPos] = proximitySensor(data.nodeID);
  if (obst) % Obstacle sensed, reverse!
    data.u1 = -data.u1;
    data.u2 = -data.u2;
    msg.node = data.nodeID;
    msg.coll = 1;
    ttSendMsg(data.master, msg, 10); % Tell base that I want new ref because
				     % of collision
    exectime = 0.0005;
    return;
  end
  ...
  refToBall=sqrt((data.xref-ballPos.xBall)^2+(data.yref-ballPos.yBall)^2);
